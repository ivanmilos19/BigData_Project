...
def convert_raw_to_formatted(file_name, current_day):
    RATING_PATH = os.path.join("datalake", "raw", "MAL", "Top_anime", current_day, file_name)
    FORMATTED_RATING_FOLDER = os.path.join("datalake","formatted", "MAL", "Top_anime", current_day)
    if not os.path.exists(FORMATTED_RATING_FOLDER):
        print("raw MAL creating folder", os.getcwd(), FORMATTED_RATING_FOLDER)
